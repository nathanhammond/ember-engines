    ...
    options.init = function() {
      ...
      this.treeForAddon = function(engineSourceTree) {
        if (this.lazyLoading === true) {
          // LAZY LOADING!
          // The only thing that we want to promote from a lazy engine is its routes.js file.
      ...
      this.treeForPublic = function() {
        ...
        // We patched over the treeForApp call, do it manually here.
        var childAppTree = buildChildAppTree.call(this);
        var childAppTreeRelocated = new Funnel(childAppTree, {
          destDir: this.name
        });

        // Add in the engine-specific config tree.
        ...
        // Move the JS in the engine tree out of modules and into the top level directory.
        // This is so that Babel does the right thing
        var engineTreeRelocated = new Funnel(engineTreeWithoutRoutes, {
          srcDir: 'modules',
          destDir: '/'
        });

        // Combine all of the engine trees which have JavaScript.
        ...
        var primaryStyleTree = concat(engineStylesTree, {
          allowNone: true,
          inputFiles: ['**/*.css'],
          outputFile: 'engines-dist/' + this.name + '/engine.css'
        });

        // Merge all of our final trees!
        return mergeTrees([publicRelocated, concatTranspiledVendorJSTree, concatTranspiledVendorStylesTree, concatTranspiledEngineTree, primaryStyleTree].filter(Boolean), { overwrite: true });
      };
