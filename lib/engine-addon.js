  ...
  extend: function(options) {
    ...
    options.init = function() {
      ...
      this.treeForPublic = function() {
        ...
        var childAddonsAddonJSTreesMoved = new Funnel(childAddonsJSTree, {
          srcDir: 'modules',
          destDir: '/'
        });
        ...
        var transpiledVendorTree = babelTranspiler(childAddonsAddonJSTreesMoved, babelOptions);
        ...
        return mergeTrees([publicMoved, concatTranspiledVendorJSTree, concatTranspiledVendorStylesTree, concatTranspiledEngineTree, primaryStyleTree, secondaryStylesTree].filter(Boolean), { overwrite: true });
      };
    ...
    */
    options.treeFor = function treeFor(name) {
      ...
      if (name === 'app') {
        trees = [];
      } else if (name === 'addon' && this.lazyLoading === true) {
        // This has to also be handled inside of the treeForPublic call.
        // Consolidate child dependencies in that namespace as opposed to shoving them into
        // the consuming app's vendor.js
        trees = [];
      } else {
